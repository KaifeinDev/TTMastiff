...
class _MyBookingScreenState extends State<MyBookingScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchBookings() async {
    ...
    try {
      ...
      for (var booking in bookings) {
        final isEnd = booking.endTime.isBefore(now) || booking.endTime.isAtSameMomentAs(now);
      ...
      sortedGroups.sort((a, b) => a.first.startTime.compareTo(b.first.startTime));
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            style: TextButton.styleFrom(foregroundColor: isLate ? Colors.orange : Colors.red),
    ...
    if (confirm == true) {
      try {
        if (isLate) {
          ...
          if(mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('已標記為請假')));
        } else {
          // > 12hr: 取消 (刪除)
          await _bookingRepo.cancelBooking(booking.id);
          if(mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('預約已取消')));
        }
        _fetchBookings(); // 重整列表
      } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('我的預約'), elevation: 0, backgroundColor: Colors.white, foregroundColor: Colors.black,
        bottom: TabBar(
          controller: _tabController,
          labelColor: Theme.of(context).primaryColor,
          unselectedLabelColor: Colors.grey,
          tabs: const [Tab(text: '即將到來'), Tab(text: '歷史紀錄')],
        ),
      ),
      body: _isLoading
...
class _GroupedBookingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final timeStr = "${timeFormat.format(session.startTime)} - ${timeFormat.format(session.endTime)}";
        ...
        children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(session.courseTitle, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      const SizedBox(height: 4),
                      Text("$dateStr  $timeStr", style: TextStyle(color: Colors.blue.shade900, fontSize: 13)),
                    ],
                  ),
                ),
              ],
          ...
          Padding(
            ...
            child: Column(
              children: bookings.map((booking) {
                // 計算是否小於 12 小時
                final hoursLeft = booking.startTime.difference(DateTime.now()).inHours;
                  ...
                  child: Row(
                    children: [
                      ...
                      CircleAvatar(
                        ...
                        child: Text(booking.student?.name[0] ?? '?', style: const TextStyle(fontSize: 12, color: Colors.black87)),
                      ),
                      const SizedBox(width: 12),
                      Text(booking.student?.name ?? '未知', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                      const Spacer(),
                      ...
                      if (isLeave)
                         Container(
                           ...
                           child: const Text('已請假', style: TextStyle(color: Colors.orange, fontSize: 12, fontWeight: FontWeight.bold)),
                         )
                      else
                        OutlinedButton(
                          ...
                          style: OutlinedButton.styleFrom(
                            ...
                            side: BorderSide(color: isLate ? Colors.orange.shade200 : Colors.red.shade200),
          ...
          if (bookings.first.session.category == 'personal')
            Padding(
              ...
              child: Text("※ 這是私人課程", style: TextStyle(color: Colors.grey.shade400, fontSize: 12)),
            ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final timeStr = "${timeFormat.format(booking.startTime)} - ${timeFormat.format(booking.endTime)}";
